...
<body class="bg-orange-50">
    ...
    <script type="text/babel">
        ...
        const App = () => {
            ...
            useEffect(() => {
                try {
                    const savedTasks = localStorage.getItem('kidsPlannerData');
                    const savedName = localStorage.getItem('kidsPlannerName');
                    const savedDates = localStorage.getItem('kidsPlannerDates');
            ...
            useEffect(() => { localStorage.setItem('kidsPlannerData', JSON.stringify(completedTasks)); }, [completedTasks]);
            useEffect(() => { localStorage.setItem('kidsPlannerName', userName); }, [userName]);
            useEffect(() => { localStorage.setItem('kidsPlannerDates', JSON.stringify(dayDates)); }, [dayDates]);
            ...
            const handleReset = () => {
                ...
                localStorage.removeItem('kidsPlannerData'); localStorage.removeItem('kidsPlannerName'); localStorage.removeItem('kidsPlannerDates');
                setShowResetConfirm(false);
            };

            const dailyTasks = [
                { id: 'word_read', category: '단어 (Word)', title: '큰 소리로 읽기', desc: '5번, 7번, 10번 읽기', icon: IconVolume2, color: 'bg-blue-100 text-blue-600' },
                { id: 'word_write', category: '단어 (Word)', title: '단어 쓰기', desc: '5번, 7번, 10번 쓰기', icon: IconPencil, color: 'bg-blue-100 text-blue-600' },
                { id: 'mb_read', category: '문장 해석 (M.B)', title: '문장 읽기', desc: '3번, 4번, 5번 읽기', icon: IconBookOpen, color: 'bg-green-100 text-green-600' },
                { id: 'mb_trans', category: '문장 해석 (M.B)', title: '문장 해석하기', desc: '3번, 4번, 5번 해석', icon: IconBookOpen, color: 'bg-green-100 text-green-600' },
                { id: 'writing_topic', category: '쓰기 (Writing)', title: '주제 쓰기 (Topic / B.R)', desc: '주제에 맞춰 글쓰기', icon: IconPencil, color: 'bg-yellow-100 text-yellow-600' },
                { id: 'writing_etc', category: '쓰기 (Writing)', title: '영어/한글 쓰기', desc: '영쓰 / 한쓰 / 기타', icon: IconPencil, color: 'bg-yellow-100 text-yellow-600' },
                { id: 'leddi_read', category: '읽기 (Leddi)', title: '집중 읽기', desc: '2, 3, 4, 5번 읽기', icon: IconBookOpen, color: 'bg-purple-100 text-purple-600' },
                { id: 'class_check', category: '마무리 학습', title: '클카 / C5 / G', desc: '학습 완료 후 체크하기', icon: IconStar, color: 'bg-pink-100 text-pink-600' },
            ];
            ...
            const triggerConfetti = () => {
                confetti({ particleCount: 100, spread: 70, origin: { y: 0.6 }, colors: ['#FFB6C1', '#87CEEB', '#FFFACD', '#E6E6FA'] });
            };
                        ...
                        <div className="bg-gradient-to-r from-orange-300 to-pink-300 p-6 rounded-b-[2.5rem] shadow-md z-10 relative">
                            <div className="flex justify-between items-start text-white mb-4">
                                <div className="flex-1">
                                    <h1 className="text-2xl font-bold tracking-tight mb-1">영어 탐험 🚀</h1>
